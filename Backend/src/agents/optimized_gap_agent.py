...
class OptimizedGapAnalyzer:
    """Optimized gap analyzer that uses embeddings for semantic matching."""
    
    def __init__(self, similarity_threshold=0.7):
        """
        Initialize the optimized gap analyzer.
        ...
        logger.info(f"Optimized gap analyzer initialized with similarity threshold: {similarity_threshold}")
    ...
    def identify_semantic_skill_gaps(self, job_skills, resume_skills):
        ...
        # Generate embeddings
        job_embeddings = self._get_embeddings(job_skill_texts, "job")
        resume_embeddings = self._get_embeddings(resume_skill_texts, "resume")
        ...
        for i, job_skill in enumerate(job_skill_texts):
            ...
            if i >= len(job_embeddings) or job_embeddings[i] is None:
                logger.warning(f"Skipping job skill due to missing embedding: {job_skill}")
                continue
            ...
            for j, resume_skill in enumerate(resume_skill_texts):
                # Skip if embedding failed
                if j >= len(resume_embeddings) or resume_embeddings[j] is None:
                    continue
    ...
    def find_similar_skills(self, skill_text, limit=5):
        ...
        try:
            ...
            # Format results
            results = [
                {"skill": skill, "similarity": float(score), "metadata": metadata}
    ...
    def _get_embeddings(self, skill_texts, source_type):
        """
        ...
        if new_skills:
            # Process in batches of 5 to reduce memory usage
            batch_size = 5
